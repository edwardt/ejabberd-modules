...
populate_table(IdMap)->
   ...
   Tab = ets:new(id_map, [named_table]),
   lists:map(fun(L)-> true = ets:insert(Tab,L) end,IdMap).

lookup_brandid(Jid)->
   UserName = jlib:jid_to_string(Jid),
   CommunityId = get_community_id(UserName),
   [{_,_,B}] = ets:match_object(id_map,{'$1',CommunityId,'$2'}),
   ?INFO_MSG("Found BrandId ~p", [B]),
   B. 

get_community_id(UserName)-> 
   case re:split(UserName,"-")	of
	[MemberId, CommunityId]->  CommunityId;
	[] -> UserName	
   end.
